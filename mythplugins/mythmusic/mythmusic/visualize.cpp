...
{
    m_size = newsize;
    scale.setMax(192, m_size.width() / analyzerBarWidth);

    rects.resize( scale.range() );
    int i = 0, w = 0;
    for (; (unsigned) i < (uint)rects.size(); i++, w += analyzerBarWidth)
    {
        rects[i].setRect(w, m_size.height() / 2, analyzerBarWidth - 1, 1);
    }
    ...
    scaleFactor = double( m_size.height() / 2 ) / log( (double)FFTW_N );
...
bool Gears::process(VisualNode *node)
{
    ...
    {
        i = node->m_length;
        fast_real_set_from_short(lin, node->m_left, node->m_length);
        if (node->m_right)
        {
            fast_real_set_from_short(rin, node->m_right, node->m_length);
        }
    } 
    else
    {
